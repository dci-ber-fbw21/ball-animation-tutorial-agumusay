...
describe("getTopLevelDomain", () => {
  ...
  test("should return top level domain if valid domain is provided", () => {
    expect(getTopLevelDomain("hello.com")).toEqual("com");
    expect(getTopLevelDomain("https://hello.com")).toEqual("com");
    expect(getTopLevelDomain("http://hello.de")).toEqual("de");
    expect(getTopLevelDomain("http://example.io")).toEqual("io");
    expect(getTopLevelDomain("http://hello.berlin")).toEqual("berlin");
    expect(getTopLevelDomain("http://sub.hello.de")).toEqual("de");
  });
});

describe("getSubdomain", () => {
  test("should return false when no url is provided", () => {
    expect(getSubdomain()).toBe(false);
  });

  test("should return false when the string provided is not an valid url", () => {
    expect(getSubdomain("https://www.hellocom")).toBe(false);
  });

  test("should return an empty string when there is no subdomain in this url", () => {
    expect(getSubdomain("https://hello.com")).toEqual("");
  });

  test("should return subdomain string when there is a subdomain in the url", () => {
    expect(getSubdomain("https://www.hello.com")).toEqual("www");
    expect(getSubdomain("https://discover.hello.com")).toEqual("discover");
    expect(getSubdomain("sub.hello.com")).toEqual("sub");
  });
});
